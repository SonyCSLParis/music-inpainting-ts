...
let scrollElementSelector: string = '.simplebar-content-wrapper';
...
export default class SheetPlaybackManager extends PlaybackManager<SheetLocator> {
    ...
    public scheduleTrackToInstrument(sequenceDuration_toneTime: Tone.TimeClass,
    ...
    private midiRequestWithData(url: string, data=null,
        method:string = 'GET'): Promise<[Midi, string]>{
    ...
    async loadMidi(serverURL: string, musicXML: XMLDocument,
        ...
        const midiBlobURL = this.midiRequestWithData(serverURL, payload, 'POST').then(([midi, blobURL])  => {
            ...
            Tone.getTransport().timeSignature = midi.header.timeSignatures[0].timeSignature;
    ...
    scheduleChordsPlayer(sheetLocator: SheetLocator, midiChannel: number) {
        ...
        const playChord = (time: number) => {
            ...
            if (currentStep % 2 == 0) {
                let chord = sheetLocator.chordSelectors[Math.floor(currentStep / 2)].currentChord;
                const events = Chord.getNoteEvents(chord.note + chord.accidental,
                    chord.chordType, time, Tone.Time('2n'), 0.5);
