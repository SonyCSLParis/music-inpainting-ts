...
export function renderFadeInControl(
    ...
    let fadeIn_duration_s: number = 0.01;
...
export class MultiChannelSpectrogramPlaybackManager extends SpectrogramPlaybackManager {
    ...
    constructor(locator: SpectrogramLocator) {
        ...
        this.voices_A = Array(this.numVoices).fill(0).map(() => {
            return new Tone.Sampler({'C4': new Tone.Buffer(this.buffer_A.get())}).connect(this.crossFade.a);
        });
        this.voices_B = Array(this.numVoices).fill(0).map(() => {
            return new Tone.Sampler({'C4': new Tone.Buffer(this.buffer_B.get())}).connect(this.crossFade.b);
        });
    }

    protected onkeydown(data: NoteEventWithChannel) {
        this.getSamplersByMidiChannel(data.channel).forEach(
            (voice) => voice.triggerAttack(data.note, undefined, data.velocity)
        );
        return this;
    }
    ...
    protected updateBuffers(): void {
        ...
        // );
        this.voices_A.forEach((voice) => {voice.dispose()});
        this.voices_A.clear();
        this.voices_A = Array(this.numVoices).fill(0).map(() => {
            return new Tone.Sampler({'C4': new Tone.Buffer(this.buffer_A.get())}).connect(this.crossFade.a);
        });
        this.voices_B.forEach((voice) => {voice.dispose()});
        this.voices_B.clear();
        this.voices_B = Array(this.numVoices).fill(0).map(() => {
            return new Tone.Sampler({'C4': new Tone.Buffer(this.buffer_B.get())}).connect(this.crossFade.b);
        });
    ...
    async loadAudio(audioURL: string): Promise<void> {
        ...
        this.voices_A.forEach((voice) => {voice.add('C4', this.buffer_A.get());});
        this.voices_B.forEach((voice) => {voice.add('C4', this.buffer_B.get());});
    }

    setFadeIn(duration_s: number) {
        super.setFadeIn(duration_s);
        this.voices_A.forEach((voice) => {voice.attack = duration_s;})
        this.voices_B.forEach((voice) => {voice.attack = duration_s;})
    }
}
