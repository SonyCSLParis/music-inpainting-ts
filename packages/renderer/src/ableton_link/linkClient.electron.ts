...
const ipcRenderer = window.ipcRendererInterface
...
export class LinkClientElectron extends AbletonLinkClient {
  ...
  protected async windowID(): Promise<number> {
    if (this._windowID == null) {
      this._windowID = <number>await ipcRenderer.getWindowId()
    }
  ...
  // ): Promise<this> {
  //   window.abletonLinkApi.on(await this.prefixMessage(message), callback.bind(this))
  //   return this
  // }
  ...
  async getPhaseAsync(): Promise<number> {
    return <Promise<number>>(
      // window.abletonLinkApi.ipcRenderer.invoke(await this.prefixMessage('get-phase'))
    )
  }
}
